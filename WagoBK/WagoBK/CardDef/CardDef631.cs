...
{
    /// <summary>
    /// configuration dialog for 631
    /// </summary>
    public partial class DlgCardDef631Properties : Form
    {
        public DlgCardDef631Properties()
        {
            InitializeComponent();
        }
        public DlgCardDef631Properties(CardDef631 Card)
        {
            m_Card = Card;
            InitializeComponent();
        ...
        private CardDef631 m_Card = null;
        private System.Windows.Forms.NumericUpDown numericUpDown1;

        private void DlgCardDef631Properties_FormClosing(object sender, FormClosingEventArgs e)
        {
            e.Cancel = true;
            this.Hide();
        }

        private void DlgCardDef631Properties_VisibleChanged(object sender, EventArgs e)
        {
    ...
    /// "2xPWM 24V 100Hz"
    /// </summary>
    public class CardDef631 : CardDefAnalogIO
    {
        ...
        public CardDef631(int Slot)
            : base(Slot)
        {
            m_OutPA.Initialize();
            m_CounterSet.Initialize();
            m_Dlgproperties = new DlgCardDef631Properties(this);
        }
        ...
        public override byte[] CreateOutPA()
        {
            ...
            {// just output PWM
                for (int i = 0; i < m_CounterSet.Length; i++)
                {
                    unscaled = ((int)(m_CounterSet[i] / m_UnitPerDigit)) * 32; //shift 5 Bit
                    m_OutPA[i * 4] = 0;
        ...
        public override void ParseOutPA(byte[] PA) {
            ...
            if (PA.Length >= m_OutPA.Length)
            {
                for (int i = 0; i < m_CounterSet.Length; i++)
                {
                    ...
                    m_CounterSet[i] = ((double)(unscaled/32)) * m_UnitPerDigit; //shift 3bit
                }
            }
        }
        ...
        private double[] m_CounterSet= new double[1];
        private double[] m_Counter = new double[1];
        private double[] m_CounterLatch = new double[1];
        private byte[] m_OutPA = new byte[6];
        private byte[] m_InPA = new byte[6];
        private SpecialFunction m_SpecialFunc;   //Flag if read/write is pending or normal operation
        private DlgCardDef631Properties m_Dlgproperties = null;
    }
}
